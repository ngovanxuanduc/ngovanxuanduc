/**
 * 2048 — pure browser, local best score.
 */
(function () {
  "use strict";
  ...
  function tileClass(v) {
    if (!v) return "g2048-cell";
    return "g2048-cell is-filled is-t" + (v > 2048 ? "super" : v);
  }
  ...
  function render() {
    for (var r = 0; r < SIZE; r++) {
      for (var c = 0; c < SIZE; c++) {
        ...
        el.setAttribute("aria-label", v ? String(v) : "trống");
      }
    }
  ...
  function slideLine(line) {
    ...
    return { line: merged, gained: gained, moved: moved };
  }
  ...
  function move(dir) {
    ...
    if (won && !over) {
      setStatus("Đạt <strong>2048</strong>! Chơi tiếp được.", "is-win");
    }

    if (!canMove()) {
      over = true;
      setStatus("Hết nước đi · <strong>" + score + "</strong> điểm", "is-draw");
    } else if (!won) {
      setStatus("Mũi tên / WASD / vuốt");
    }
    return true;
  }
  ...
  document.addEventListener("keydown", function (e) {
    ...
    var dir = map[e.key];
    if (!dir) return;
    // only when board in view / not typing
