(function () {
  ...
  var btnRun = document.getElementById("jwt-run");
  var btnCopy = document.getElementById("jwt-copy");
  var btnClear = document.getElementById("jwt-clear");
  if (!inn) return;
  ...
  function b64urlToJson(part) {
    if (part == null || part === "") throw new Error("Phần JWT trống");
    ...
    try {
      ...
      throw new Error("Base64URL không hợp lệ");
    }
  ...
  function run() {
    try {
      var parts = String(inn.value || "")
        .trim()
        .split(".");
      if (parts.length < 2) throw new Error("JWT cần ≥ 2 phần (header.payload)");
      if (header) header.value = b64urlToJson(parts[0]);
      if (payload) payload.value = b64urlToJson(parts[1]);
      setMeta(
        parts.length === 3
  ...
  if (btnCopy) {
    ...
    btnCopy.addEventListener("click", function () {
      var text = (payload && payload.value) || "";
