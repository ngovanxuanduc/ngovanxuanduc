/**
 * Minesweeper — easy / medium / hard, flag + chord.
 */
(function () {
  "use strict";
  ...
  var diffEl = document.getElementById("ms-diff");
  ...
  var cols = 12;
  var mineCount = 24;
  var cells = []; // { mine, open, flag, adj, el }
  var started = false;
  var over = false;
  var flags = 0;
  var timer = null;
  ...
  function paint(cell) {
    ...
    if (cell.adj > 0) {
      el.classList.add("is-n" + cell.adj);
  ...
  function placeMines(safeR, safeC) {
    ...
    while (placed < mineCount) {
      ...
      // keep first-click neighborhood safe
    ...
    for (var i = 0; i < cells.length; i++) {
      ...
      neighbors(rr, cc).forEach(function (p) {
        if (cells[idx(p.r, p.c)].mine) n++;
      });
      cell2.adj = n;
    }
  }

  function openFlood(r, c) {
    var stack = [{ r: r, c: c }];
    while (stack.length) {
      var p = stack.pop();
      if (!inBounds(p.r, p.c)) continue;
      var cell = cells[idx(p.r, p.c)];
      if (cell.open || cell.flag) continue;
      cell.open = true;
      if (!cell.mine && cell.adj === 0) {
        neighbors(p.r, p.c).forEach(function (n) {
          stack.push(n);
        });
      }
    }
  ...
  function checkWin() {
    for (var i = 0; i < cells.length; i++) {
      if (!cells[i].mine && !cells[i].open) return false;
    }
    return true;
  }
  ...
  function revealMines() {
    for (var i = 0; i < cells.length; i++) {
      if (cells[i].mine) cells[i].open = true;
    }
  }
  ...
  function lose(hit) {
    ...
    stopTimer();
    revealMines();
    if (hit) hit.el.classList.add("is-hit");
    paintAll();
    setStatus("Nổ rồi 💣 · " + seconds + "s", "is-draw");
  }
  ...
  function win() {
    ...
    paintAll();
    setStatus("Thắng! 🎉 · " + seconds + "s", "is-win");
  }
  ...
  function chord(r, c) {
    ...
    var neigh = neighbors(r, c);
    var flagged = 0;
    neigh.forEach(function (p) {
      if (cells[idx(p.r, p.c)].flag) flagged++;
    });
    if (flagged !== cell.adj) return;
    neigh.forEach(function (p) {
      var n = cells[idx(p.r, p.c)];
      if (!n.flag && !n.open) openCell(p.r, p.c);
    });
  }
  ...
  function newGame() {
    stopTimer();
    var preset = PRESETS[(diffEl && diffEl.value) || "medium"] || PRESETS.medium;
    rows = preset.rows;
    cols = preset.cols;
    ...
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c < cols; c++) {
        ...
        el.type = "button";
        el.className = "ms-cell is-closed";
        el.setAttribute("aria-label", "Ô " + (r + 1) + "," + (c + 1));
        bindCell(el, r, c);
    ...
    updateHud();
    setStatus("Click ô để bắt đầu");
  }

  if (newBtn) newBtn.addEventListener("click", newGame);
  if (diffEl) diffEl.addEventListener("change", newGame);
